...
var numofseeds;
var turn = true;
var names = true; var p1name = ""; var p2name = "";
var AudioWin = new Audio('sounds/mancwin.wav');
var AudioTie = new Audio('sounds/manctie.mp3');
var mancala =
    [
        [4, 4, 4, 4, 4, 4, 0],
        [0, 4, 4, 4, 4, 4, 4],
    ...
    for (row = 0; row < 2; row++) {
        ...
        for (col = 0; col < 7; col++) {
            if ((row === 0 && col === 6) || (row === 1 && col === 0)) {
                if (row === 0 && col === 6) {
                    if (mancala[row][col] < 45)
                        textspecial = textspecial + "<div class='containerStore'>" + "<div class='storep1' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + "</div> <img src = '" + ImgArray[mancala[row][col]] + "'  width = '80%'; height = '50%' /> </div > " + "</div>";
                    else
                        textspecial = textspecial + "<div class='containerStore'>" + "<div class='storep1' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + "</div> <img src = '" + ImgArray[45] + "'  width = '80%'; height = '50%' /> </div > " + "</div>";

                }
                else if (row === 1 && col === 0) {
                    if (mancala[row][col] < 45)
                        textspecial = textspecial + "<div class='containerStore'>" + "<div class='storep2' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + " </div> <img src = '" + ImgArray[mancala[row][col]] + "'  width = '80%'; height = '50%' /> </div > " + "</div>";
                    else
                        textspecial = textspecial + "<div class='containerStore'>" + "<div class='storep1' id='" + num + "' ;' onclick='StartGame(this);'> <div class='label'>" + mancala[row][col] + "</div> <img src = '" + ImgArray[45] + "'  width = '80%'; height = '50%' /> </div > " + "</div>";
                }
                num++;
...
function IsEndGame() {
    ...
    if (nonep1 || nonep2) {
        ...
        if (mancala[0][6] > mancala[1][0]) {
            ...
            document.getElementById("End").outerHTML += "</br> <div class='containerend'>" + "<div class='endfont'>  <div class='player1name'>" + p1name + "</div>" + " captured " + mancala[0][6] + " seeds </div>" + "</div>";
            ...
            AudioTie.play();
            document.getElementById("Start").outerHTML = "  ";
            document.getElementById("End").outerHTML += "<h1 style='text-align:center; color:purple'> Both of you captured " + mancala[0][6] + " seeds </h1 > ";
        }
        else {
            alert("Player 2 won!");
